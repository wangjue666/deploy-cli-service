...
// 是否确认部署
const confirmDeploy = (message, isSkipPrompt) => {
  if (isSkipPrompt) {
    log(message)
...
const executeSSHCommand = async (config, index) => {
  try {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      const execCommandName = command.command || command
      log(`(${index}) 执行远程命令 ${underline(execCommandName)}`)
      let res
      if (command instanceof Object) {
        res = await ssh.execCommand(command.command, command.options)
      } else {
        res = await ssh.execCommand(command)
      }

      log(`(${execCommandName}) 执行结果： `)
      if (res.stdout) {
        log(res.stdout)
      } else {
        log(res.stderr)
